...
1. On 0?0: sign in, then `CREATE ARCHIVE` (or Create Archive) — opens the template.
2. Rename **Anon Archive**, add a short description, **Save & create**, confirm.
3. Optional: for a static repo folder, copy this `templates/` tree → `archives/Your-Archive-Name/` and edit `data.json`.
4. Live rooms created on 0?0 open from the registry immediately.

See RTFM **Archive Creation Manual** for the complexity tree.
